    ...
    with torch.no_grad():
        for ([clean_name], degrad_patch, clean_patch) in tqdm(testloader):
            ...
            transform = torchvision.transforms.Compose([
                Resize(
                width=degrad_patch.shape[-1],
                height=degrad_patch.shape[-1],
                resize_target=False,
            ...
            degrad_patch = torch.from_numpy(transform({'image': degrad_patch.cpu().squeeze().permute(1, 2, 0).numpy()})['image']).unsqueeze(0).permute(0, 3, 1, 2).cuda()
            clean_patch = torch.from_numpy(transform({'image': clean_patch.cpu().squeeze().permute(1, 2, 0).numpy()})['image']).unsqueeze(0).permute(0, 3, 1, 2).cuda()


            restored = net.infer_image(x_query=degrad_patch, x_key=degrad_patch, input_size=degrad_patch.shape[-1])
...
def test_Derain_Dehaze(net, dataset, task="derain"):
    ...
    with torch.no_grad():
        for ([degraded_name], degrad_patch, clean_patch) in tqdm(testloader):
            ...
            restored = net.infer_image(x_query=degrad_patch, x_key=degrad_patch)
            temp_psnr, temp_ssim, N = compute_psnr_ssim(restored, clean_patch)
            psnr.update(temp_psnr, N)
            ssim.update(temp_ssim, N)
